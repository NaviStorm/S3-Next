                        ...
                        if selectedTab == 0 {
                            ...
                            HStack(alignment: .top, spacing: 15) {
                                RibbonGroup(label: "Navigation") {
                                    RibbonButton(
                                        title: "Racine", icon: "house.fill", action: onNavigateHome)
                                    RibbonButton(
                                        title: "Retour", icon: "arrow.left.circle.fill",
                                        action: onNavigateBack,
                                        disabled: appState.currentPath.isEmpty)
                                    RibbonButton(
                                        title: "Actualiser", icon: "arrow.clockwise.circle.fill",
                                        action: onRefresh)
                                }
                                ...
                                RibbonGroup(label: "Chiffrement (Sticky)") {
                                    Menu {
                                        ...
                                        VStack(spacing: 4) {
                                            ...
                                            Text(appState.selectedEncryptionAlias ?? "Clé active")
                                                .font(.system(size: 11))
                                                .lineLimit(1)
                                        }
                                        .frame(width: 70)
                            ...
                            HStack(alignment: .top, spacing: 15) {
                                ...
                                RibbonGroup(label: "Configuration") {
                                    VStack(alignment: .leading, spacing: 2) {
                                        HStack {
                                            Image(systemName: "archivebox.fill").foregroundColor(
                                                .secondary)
        ...
        Button(action: action) {
            VStack(spacing: 4) {
                Image(systemName: icon)
                    .font(.system(size: 20))
                    .foregroundStyle(disabled ? AnyShapeStyle(.secondary) : AnyShapeStyle(color))
